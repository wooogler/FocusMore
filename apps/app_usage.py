...
@app.callback(Output("selectDay", "options"),
            [Input("date-picker-range","start_date"),
            Input("date-picker-range","end_date")])
def update_dropdown(start_date, end_date):
    return [{"label": i, "value": i} for i in pd.date_range(start_date, end_date, freq='D').format(formatter=lambda x: x.strftime('%Y-%m-%d'))]


@app.callback([Output("FocusBar", "figure"),
...
def update_graphs(user_name, start_date, selectedDay):
    ...
    idx = actDf.groupby(['timestamp'])['confidence'].transform(max) == actDf['confidence']
    ...
    for i, r in actDf_byDay.iterrows():
        ...
        if i == 0:
            ...
            if (r['hour'] > actDf_byDay.loc[i - 1]['hour'] and r['label'] == 1) or (actDf_byDay.loc[i - 1]['label'] == 0 and r['label'] == 1):
                still_start = r.timestamp
            ...
            elif (r['hour'] < actDf_byDay.loc[i + 1]['hour']) or (actDf_byDay.loc[i - 1]['label'] == 1 and r['label'] == 0):
                still_end = r.timestamp
    ...
    if selectedDay == "Day":
        screenDf_byDay = screenDf.loc[screenDf['time'].dt.date.astype(str) == start_date]
    else:
        screenDf_byDay = screenDf.loc[screenDf['time'].dt.date.astype(str) == selectedDay]
    ...
    for s, t in still_inters:
        screenDf_byDay_interv = screenDf_byDay.loc[lambda x: x['timestamp'].between(s, t), :]
        screenDf_byDay_interv = screenDf_byDay_interv.sort_values('timestamp').reset_index(drop=True)
    ...
    screenDf_byDay_still = screenDf_byDay_still.sort_values('timestamp').reset_index(drop=True)
    ...
    ratio_df = pd.DataFrame({'timeslot':list(focus_ratio.keys()), 'ratio': list(focus_ratio.values())})

    meanVal = ratio_df['ratio'].mean()
    ratio_df['label'] = ratio_df["ratio"].apply(lambda x: "High" if x > meanVal else "Low")
    ...
    appDf_byDay_high = appDf_byDay.loc[appDf_byDay['hour'].isin(ratio_df_high['timeslot'])]
    appDf_byDay_low = appDf_byDay.loc[appDf_byDay['hour'].isin(ratio_df_low['timeslot'])]
    ...
    highTop5 = pd.concat([highTop5, pd.DataFrame([['etc',highEtc]], columns = ['name', 'totalTimeForeground'])])
    ...
    lowTop5 = pd.concat([lowTop5, pd.DataFrame([['etc',lowEtc]], columns = ['name', 'totalTimeForeground'])])
